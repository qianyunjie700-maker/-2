...
export const Dashboard: React.FC = () => {
  ...
  const [currentDeptIndex, setCurrentDeptIndex] = useState(-1); // -1 means "All Departments"
      ...
      setTimeLeft((prev) => {
        if (prev <= 1) {
          setCurrentDeptIndex((prevIndex) => {
            ...
            return nextIndex >= DEPARTMENTS.length ? -1 : nextIndex;
          });
          return 20; // 重置计时器
        }
        return prev - 1;
  ...
  const currentDeptName = currentDeptIndex === -1 ? '集团总部 // 全局总览' : DEPARTMENTS[currentDeptIndex].name;
  
  // 直接在组件内部计算统计数据，避免调用store的getStats方法
  const departmentKey = currentDeptIndex === -1 ? undefined : DEPARTMENTS[currentDeptIndex].key;
  const targetOrders = departmentKey 
    ? orders.filter(o => o.department_key === departmentKey && !o.is_archived)
  ...
  targetOrders.forEach(o => {
    ...
    if (o.warning_status === WarningStatus.TRANSIT_ABNORMAL) {
  ...
  if (orders.length > 0) {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      // 统计当天的发货订单数量（状态为delivered的订单）
      const deliveredCount = orders.filter(order => {
        const orderDate = order.updated_at.split('T')[0];
        return orderDate === dateStr && order.status === OrderStatus.DELIVERED;
      }).length;
      
      // 统计当天的异常订单数量（有预警状态的订单）
      const riskCount = orders.filter(order => {
        const orderDate = order.updated_at.split('T')[0];
        return orderDate === dateStr && order.warning_status !== WarningStatus.NONE;
      }).length;
  ...
  const riskOrders = orders.filter(o => {
    return !o.is_archived && o.warning_status !== WarningStatus.NONE;
  });
    ...
    <div className="h-full flex flex-col font-sans overflow-hidden">
        ...
        <div>
          <div className="flex items-center space-x-3">
             ...
             <h2 className="text-2xl font-bold text-white tracking-widest font-mono uppercase">
                {currentDeptIndex === -1 ? '全集团物流总览' : DEPARTMENTS[currentDeptIndex].name}
             </h2>
          </div>
          <div className="flex items-center space-x-4 mt-1">
              ...
              <div className="w-px h-3 bg-cyan-900"></div>
              <p className="text-xs text-slate-500 font-mono">轮播倒计时: <span className="text-cyan-400 font-bold">{timeLeft.toString().padStart(2, '0')}s</span></p>
          </div>
        </div>
      ...
      <div className="flex-1 p-4 overflow-hidden grid grid-rows-[auto_1fr] gap-4 min-h-0">
        ...
        <div className="grid grid-cols-6 gap-4 flex-shrink-0">
          ...
          <StatCard 
            title="运输异常" 
            count={stats.transitAbnormalCount} 
            total={stats.warningCount} 
            colorClass="pink-500" 
