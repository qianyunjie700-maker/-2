      ...
      try {
        ...
        for (const header of requiredHeaders) {
          ...
          if (!found) {
            for (const h of normalizedHeaders) {
              if (header === '申请单号/外部订单号' && (h.includes('申请单号') || h.includes('外部订单号'))) {
                found = true;
                break;
        ...
        const rows = jsonData.slice(1).map((row: any[]) => {
          ...
          headers.forEach((header, index) => {
            ...
            // 为常用字段添加别名映射
            if (normalizedHeader.includes('申请单号') || normalizedHeader.includes('外部订单号')) {
              rowObj['申请单号/外部订单号'.replace(/\s+/g, '').toLowerCase()] = row[index];
            } else if (normalizedHeader.includes('客户') || normalizedHeader.includes('项目名称')) {
              rowObj['客户/项目名称'.replace(/\s+/g, '').toLowerCase()] = row[index];
...
export const processImportData = (rawData: any[]): { orders: Order[]; errors: Array<{ row: number; message: string }> } => {
  ...
  rawData.forEach((row, index) => {
    ...
    if (recipient.length < 2) {
      errors.push({ row: rowNumber, message: '收货人名称必须至少包含2个字符' });
      return;
    }
    ...
    // ✅ 修复：移除订单号重复检查，因为订单号是可以重复的
    const orderNumber = String(getField('订单号')).trim();
    
    // ✅ 修复：添加快递单号格式验证
    const carrierName = String(getField('快递公司')).trim();
    const carrierCode = CARRIER_NAME_TO_CODE[carrierName] || '';
    const trackingNumber = String(getField('快递单号')).trim();
